...
/**
 * @brief The function call isn't allowed according to RF4CE context
 * (mac mode, normal mode)
 */
#define RF4CE_INVALID_CALL 0x105
...
typedef struct NLME_PAIR_CONFIRM_S {
  ...
  /** The user defined identification string of
    *  the originator of the
    *  pair response.
    */
...
/**
  * @brief MAC packet parameter, used when the RF4CE network layer is in bootlader mode
  */
typedef struct macPacketS {
  /** Physical channel (15-20-25)
    *  where to send the message
    */
  uint8_t channel;
  /** If this subfield is set to one,
    *  the recipient device shall send an acknowledgment
    *  frame only if, upon reception, the frame passes
    *  the filtering
    */
  bool ackRequired;
  /** Source address mode
    *  - 2 Address field contains a 16-bit short address.
    *  - 3 Address field contains a 64-bit extended address.
    */ 
  uint8_t srcAddrMode;
  /** The address of the source device.
    *  According to the srcAddrMode parameter
    */
  union macSrcAddr_U {
    /** If SrcAddrMode = 2, this is the short address
      */
    uint16_t ShortAddress;
    /** If SrcAddrMode = 3, this is the IEEE address
      */
    IEEEAddr IEEEAddress;
  } macSrcAddr;
  ...
  /** The address of the destination device.
    ...
    */
  union macDstAddr_U {
    /** If DstAddrMode = 2, this is the short address
      */
    uint16_t ShortAddress;
    /** If DstAddrMode = 3, this is the IEEE address
      */
    IEEEAddr IEEEAddress;
  } macDstAddr;
  ...
  uint16_t dstPANId;
  /** Payload length.
    *  Max length is 127 bytes included
    *  MAC header and MAC CRC*/
...
/**
  * @brief rawPacketReceived parameters
  */
typedef struct rawPacketReceived_S {
  /** The Frame Control field is 2 octets in length and contains information defining the frame type,
    *  addressing fields, and other control flags.
    *  control flags.
    */
  struct rawFrameControl_B {
    /** Frame type:
      *  - 0 Beacon
      *  - 1 Data
      *  - 3 MAC command 
    ...
    /** The Security Enabled subfield is 1 bit in length, and it shall be set to one if the frame 
      *  is protected by the MAC sublayer and shall be set to zero otherwise. 
      */
    uint16_t SecurityEnabled : 1;
    /**  The Frame Pending subfield is 1 bit in length and shall be set to one if the device sending
      *   the frame has more data for the recipient. This subfield shall be set to zero otherwise
      */
    ...
    /** Destinaton address mode
      ...
      *  - 3 Address field contains a 64-bit extended address.
      */
    uint16_t DestinationAddressingMode : 2;
    /** The Frame Version subfield is 2 bits in length and specifies the version number corresponding to the frame.
      ...
      * indicate an IEEE 802.15.4 frame 
      */
  ...
  /** The Destination PAN Identifier field, when present, is 2 octets in length and specifies the unique PAN
    *  identifier of the intended recipient of the frame.
    */
  uint16_t DestinationPANIdentifier;

  /** The Destination Address field, when present, is either 2 octets or 8 octets in length, according to
    *  the value specified in the Destination Addressing Mode subfield of the Frame Control field
    */
  union rawDestinationAddress_U {
    /** If DestinationAddressingMode = 2, this is the short address 
      */
    uint16_t networkAddress;
    /** If DestinationAddressingMode = 3, this is the IEEE address
      */
    IEEEAddr IEEEAddress;
  ...
  /** The Source Address field, when present, is either 2 octets or 8 octets in length, according to
    *  the value specified in the Source Addressing Mode subfield of the Frame Control field
    */   
  union rawSourceAddress_U {
    /** If SourceAddressingMode = 2, this is the short address
      */
    uint16_t networkAddress;
    /** If SourceAddressingMode = 3, this is the IEEE address
      */
    IEEEAddr IEEEAddress;
  } rawSourceAddress;
  ...
  /** RSSI */
  int8_t rssi;
  /** Channel on which the packet has been received */
  uint8_t channel;
} rawPacketReceivedType;
...
/** @brief  Helper function to disable RF part of the device
  ...
  * @note   This function is equivalent to call NLME_RX_ENABLE_request(0) and its 
...
/** @brief  Helper function to configure RF4CE network layer in mac mode
  * @param  enable  Flag to enable/disable the mac mode
  * @note   If the mac mode is enabled the RF4CE network request aren't served
  * @retval Status of the operation. Valid values are: SUCCESS, otherwise other RF4CE nwk status
  */
uint32_t NWK_EnableMACMode(bool enable);

/** @brief  This function signals a new mac message received by the application
  * @param  msg   MAC packet received
  * @retval None
  * @note   This function is a callback and must be provided by the application. Enabled only when the RF4CE
  *   is in MAC mode, it is called inside an iterrupt service routine  
  */
extern void NWK_IncomingMACMessageHandler(rawPacketReceivedType *msg);

/** @brief  This function send a MAC raw packet
  * @param  packet This structure contains all the info necessary to send a raw packet to a destination device
  * @retval Status of the request.
  * @note   This function is enabled only if the mac mode is enabled  
  */
uint32_t NWK_SendMACPacket(macPacket_Type *packet);

/** @brief  This function signals the outcome of the raw packet sent
  * @param  status Status of the packet transmission
  * @retval None
  * @note   This function is a callback and must be provided by the application. Enabled only when the RF4CE
  *   is in mac mode    
  */
extern void NWK_TransmitMACMessageHandler(uint32_t status);

/** @brief  Helper function to get the EUI64 from the RF4CE network layer
  * @param  eui64 variable contains the long address 
  * @return SUCCESS
  */
uint32_t NWK_getEui64(uint8_t *eui64);

/** @brief  Helper function to get the TX power
  * @param  power variable contains the TX power
  * @retval SUCCESS
  */
uint32_t NWK_getTxPower(int8_t *power);

/** @brief  Helper function to set the TX power
  * @param  power variable contains the TX power
  * @retval SUCCESS
  */
uint32_t NWK_setTxPower(int8_t power);
