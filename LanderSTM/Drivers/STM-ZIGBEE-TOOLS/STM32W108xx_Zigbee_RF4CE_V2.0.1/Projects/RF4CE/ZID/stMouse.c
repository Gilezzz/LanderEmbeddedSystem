...
static uint32_t cfgDevice(bool host, bool coldStart)
{
  ...
  status = ZID_DeviceInit(host, coldStart);
...
static bool processBoardInfo(void)
{
  ...
  if (((thisMillisecondTick - lastMillisecondTick) >= 9) && 
      (LIS302DL_GetStatus() & (STATUS_REG_ZYXDA))) {
...
int main (void)
{
  ...
  printf("RF4CE ZID application profile version %s (lib version:%s)\r\n", ZID_RF4CE_VERSION_STRING, RF4CE_VERSION_STRING);
...
void ZID_DeviceInitCallback(uint32_t *status)
{
  ...
  configureLed(led);
  printf("ZID_DeviceInitCallback ... DeviceType 0x%02x status 0x%08x\r\n", statusDev.nodeType, *status);
...
{
  printf("DELIVERY STATUS... CommandID = 0x%02x Status = 0x%08x PairRef = 0x%02x\r\n",
