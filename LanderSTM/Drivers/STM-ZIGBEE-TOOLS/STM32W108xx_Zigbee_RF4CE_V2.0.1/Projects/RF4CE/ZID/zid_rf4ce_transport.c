/**
  ******************************************************************************
  * @file    zid_rf4ce_transport.c
  * @author  MCD Application Team
  * @version V2.0.1
  * @date    30-November-2012 
  * @brief   This file provides the HID RF4CE usb transport firmware 
...
uint32_t sendPacket(uint8_t *packet, uint16_t length)
{
  ...
  printf("Packet to send with length=%d :", length); 
  for (i=0; i<length; i++) {
  ...
  startTime = currentMilliSecondTime();

  for (i=0; i<length; i++) {
    while (!((SC1_UART->UARTSR >> UART_TXFREE)&0x01) && (timeMilliSecondElapsed(startTime)< UART_TIMEOUT));

    if (!((SC1_UART->UARTSR >> UART_TXFREE)&0x01))
      return SERIAL_TX_TIMEOUT;

    /* Queued data in serial TX FIFO */
    SC1_UART->DR = packet[i];
  }
#endif
