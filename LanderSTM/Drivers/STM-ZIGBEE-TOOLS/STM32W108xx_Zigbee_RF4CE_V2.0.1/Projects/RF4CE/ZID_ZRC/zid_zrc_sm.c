/**
  ...
  * @brief   zid_zrc state machine program body
  ******************************************************************************
  * @attention
  *
...
static void verifyCmdResponse(uint8_t pairRef, uint8_t cmdCode, uint8_t profileId);
#ifdef ZID_FULL
...
#endif
static bool isZRCSupported(configurationContextType *context);
static bool isZIDSupported(configurationContextType *context);
...
static bool isZRCSupported(configurationContextType *context) {
  ...
  for (i=0; i<context->numProfile; i++) {
    if ((context->profileList[i] == ZRC_APP_PROFILE) && 
	(context->profileSupported & ZRC_APP_PROFILE)) {
      returnValue = TRUE;
    }
  }
  return returnValue;
...
static bool isZIDSupported(configurationContextType *context) {
  ...
  for (i=0; i<context->numProfile; i++) {
    if ((context->profileList[i] == ZID_APP_PROFILE) && 
	(context->profileSupported & ZID_APP_PROFILE)) {
      returnValue = TRUE;
    }
  }
  return returnValue;
...
static void verifyCmdResponse(uint8_t pairRef, uint8_t cmdCode, uint8_t profileId)
{
  ...
  if (profileId == ZRC_APP_PROFILE) {
    if ((cmdQ[pairRef].cmd == COMMAND_DISCOVERY_REQUEST_CODE) && 
	(cmdCode == COMMAND_DISCOVERY_RESPONSE_CODE)) {
      cmdQ[pairRef].cmd = 0;
    }
  }
}
...
static GRSP_Status extractHIDAttributes(void)
{
  ...
  while(index < length) {
    ...
    if ((attr.id >= aplHIDNonStdDescCompSpec_MIN_ID) && attrOk) {
      ...
      /* memcpy(&descSize, &context->msg[4], 2); ??? */
	...
	/* memcpy(&context->nonStdDesc[1], &descSize, 2); ??? */
...
static void callbackSchedule(NLDE_DATA_INDICATION_Type *param)
{
  ...
  if (profileId == ZRC_APP_PROFILE) {
	...
	press.pairRef = param->PairingRef;
	press.commandCode = param->nsdu[1];
	if (param->nsduLength > 2) {
	  press.commandPayload = &param->nsdu[2];
	  press.payloadLength = param->nsduLength - 2;
	} else {
	  press.payloadLength = 0;
	}
	cmdCode = USER_CONTROL_PRESSED_CODE;
	ZRC_CommandNotification(cmdCode, &press);
      }
      break;
	...
	cmdCode = USER_CONTROL_REPEATED_CODE;
	ZRC_CommandNotification(cmdCode, &press);
      }
      break;
	...
	release.pairRef = param->PairingRef;
	release.commandCode = param->nsdu[1];
	cmdCode = USER_CONTROL_RELEASED_CODE;
	ZRC_CommandNotification(cmdCode, &release);
      }
      break;
    case COMMAND_DISCOVERY_REQUEST_CODE:
      {
	CommandDiscoveryResponse_Type dscRsp;
	
	dscRsp.pairRef = param->PairingRef;
	ZRC_CommandDiscoveryResponse(&dscRsp);
      }
      break;
	...
	cmdCode = COMMAND_DISCOVERY_RESPONSE_CODE;
	dscRsp.pairRef = param->PairingRef;
	memcpy(dscRsp.commandSupported, &param->nsdu[2], 32);
	ZRC_CommandNotification(cmdCode, &dscRsp);
      }
      break;
    }
  }
...
uint32_t pairingStateMachine(uint32_t subState)
{
  ...
  case HID_SUBSTATE_PAIRING_DSC_REQ:
    {
      ...
      switch (context->profileSupported) {
      case ZRC_APP_PROFILE:
	{
	  dscParam.OrgAppCapabilities = 0x13; //User string - 1 Dev supported - 1 Profile Supported
	  dscParam.OrgProfileIdList[0] = 0x01; //ZigBee Remote Control (ZRC)
	  dscParam.DiscProfileIdListSize = 1;
	  dscParam.DiscProfileIdList[0] = 0x01; //ZigBee Remote Control (ZRC)
	}
	break;
      case ZID_APP_PROFILE:
	{
	  dscParam.OrgAppCapabilities = 0x13; /* User string - 1 Dev supported - 1 Profile Supported */
	...
	break;
      case (ZID_APP_PROFILE | ZRC_APP_PROFILE):
	{
	  dscParam.OrgAppCapabilities = 0x23; /* User string - 1 Dev supported - 2 Profile Supported */
	  dscParam.OrgProfileIdList[0] = 0x01; /* ZigBee Remote Control (ZRC) */
	  dscParam.OrgProfileIdList[1] = 0x02; /* ZigBee Input Device (ZID) */	  
	  dscParam.DiscProfileIdListSize = 2;
	  dscParam.DiscProfileIdList[0] = 0x01; /* ZigBee Remote Control (ZRC) */
	  dscParam.DiscProfileIdList[1] = 0x02; /* ZigBee Input Device (ZID) */
	}
	break;
      }
  ...
  case HID_SUBSTATE_PAIRING_PAIR_REQ:
	...
	numberOfKey = aplcMinKeyExchangeTransferCount;
      ...
      case ZRC_APP_PROFILE:
	{
	  param.OrgAppCapabilities = 0x13; /* User string - 1 Dev supported - 1 Profile Supported */
	  param.OrgProfileIdList[0] = 0x01; /* ZigBee Remote Control (ZRC) */
	}
	break;
      case ZID_APP_PROFILE:
	{
	...
	break;
      case (ZID_APP_PROFILE | ZRC_APP_PROFILE):
	{
	  param.OrgAppCapabilities = 0x23; /* User string - 1 Dev supported - 2 Profile Supported */
	  param.OrgProfileIdList[0] = 0x01; /* ZigBee Remote Control (ZRC) */
	  param.OrgProfileIdList[1] = 0x02; /* ZigBee Input Device (ZID)	*/  
	}
	break;
      }
  ...
  case HID_SUBSTATE_PAIRING_AUTODSC_REQ:
      ...
      case ZRC_APP_PROFILE:
	{
	  ...
	  dscParam.RecProfileIdList[0] = 0x01; /* ZigBee Remote Control (ZRC) */
	}
	break;
      case ZID_APP_PROFILE:
	{
	...
	break;
      case (ZID_APP_PROFILE | ZRC_APP_PROFILE):
	{
	  dscParam.RecAppCapabilities = 0x23; /* User string - 1 Dev supported - 2 Profile Supported */
	  dscParam.RecProfileIdList[0] = 0x01; /* ZigBee Remote Control (ZRC) */
	  dscParam.RecProfileIdList[1] = 0x02; /* ZigBee Input Device (ZID) */	  
	}
	break;
      }
  ...
  case HID_SUBSTATE_PAIRING_PAIR_RSP:
      ...
      case ZRC_APP_PROFILE:
	{
	  ...
	  param.RecProfileIdList[0] = 0x01; /* ZigBee Remote Control (ZRC) */
	}
	break;
      case ZID_APP_PROFILE:
	{
	...
	break;
      case (ZID_APP_PROFILE | ZRC_APP_PROFILE):
	{
	  param.RecAppCapabilities = 0x23; /* User string - 1 Dev supported - 2 Profile Supported */
	  param.RecProfileIdList[0] = 0x01; /* ZigBee Remote Control (ZRC) */
	  param.RecProfileIdList[1] = 0x02; /* ZigBee Input Device (ZID) */	  
	}
	break;
      }
...
uint32_t configurationStateMachine(uint32_t subState)
{
  ...
  /* ZRC Configuration */
  if (isZRCSupported(context)) {
    switch(context->zrcState) {
    case ZRC_CONFIGURATION_INIT:
      {
	if (!isHIDAdapter()) { /* Controller */
	  if ((TIME_ELAPSED(context->startTime) >= aplcMaxCmdDiscRxOnDuration) || context->msgRcv) {
	    context->startTime = TIME_CurrentTime();
	    context->zrcState = ZRC_CONFIGURATION_BLACKOUT_TIME;
	  }
	} else { /* Target */
	  CommandDiscoveryRequest_Type dscReq;
	  dscReq.pairRef = context->pairRef;
	  ZRC_CommandDiscoveryRequest(&dscReq);
	  context->startTime = TIME_CurrentTime();
	  context->zrcState = ZRC_CONFIGURATION_WAIT_RSP;
	}
      }
      break;
    case ZRC_CONFIGURATION_WAIT_RSP:
      {
	if ((TIME_ELAPSED(context->startTime) >= aplcMaxResponseWaitTime) || context->msgRcv) {
	  context->zrcState = ZRC_CONFIGURATION_BLACKOUT_TIME;
	}
      }
      break;
    case ZRC_CONFIGURATION_BLACKOUT_TIME:
      {
	context->msgRcv = FALSE;
	if (TIME_ELAPSED(context->startTime) >= aplcConfigBlackoutTime)
	  context->profileSupported &= 0xfe; /* Delete ZRC support because already served */
      } 
      break;
    }
  } else {
    /* ZID Configuration */
    if (isZIDSupported(context)) {
      switch (subState) {
    ...
    } else { /* END ZID Configuration Support */
      context->configurationSuccess = TRUE;
      SUBSTATE_TRANSITION(HID_SUBSTATE_CONFIGURATION, INIT, DONE); /* Nothing else to do */
    }
  }
...
HID_StateType hidStateMachine(HID_StateType state)
{
  ...
  case HID_STATE_START:
    {
      ...
      idleContext.profileSupported = context->profileSupported;
	...
	if ((!context->startComplete) || (context->status != SUCCESS)) {
	  ...
	  if (devIsPaired()) {
	    ...
	    if (isHIDAdapter() && (idleContext.profileSupported & ZID_APP_PROFILE)) {
	      proxyContextType *context =  (proxyContextType *) hidGlobalContext;
  ...
  case HID_STATE_PAIRING:
    {
      pairingContextType *context =  (pairingContextType *) hidGlobalContext;
      context->profileSupported = idleContext.profileSupported;
      newState.subState = pairingStateMachine(state.subState);
      if (newState.subState == HID_SUBSTATE_PAIRING_DONE) {
	if (context->pairingStatus == SUCCESS) {
	  uint8_t pairRef, numProfileSupp;
	  uint8_t profileList[7];
	  pairRef = context->newPairRef;
	  numProfileSupp = context->numProfile;
	  memcpy(profileList, context->profileList, numProfileSupp);	  
	  { 
	    configurationContextType *context =  (configurationContextType *) hidGlobalContext; 
	    context->msgRcv = FALSE;
	    context->pairRef = pairRef;
	    context->profileSupported = idleContext.profileSupported;
	    context->numProfile = numProfileSupp;
	    memcpy(context->profileList, profileList, numProfileSupp);
	    context->zrcState = ZRC_CONFIGURATION_INIT;
	    context->startTime = TIME_CurrentTime();
	  }
...
void NLME_PAIR_confirm(NLME_PAIR_CONFIRM_Type *param)
{
  ...
  context->numProfile = (param->RecAppCapabilities & 0x70) >> 4;
  memcpy(context->profileList, param->RecProfileIdList, context->numProfile);
...
void NLME_PAIR_indication (NLME_PAIR_INDICATION_Type *params)
{
  ...
  context->numProfile = (params->OrgAppCapabilities & 0x70) >> 4;
  memcpy(context->profileList, params->OrgProfileIdList, context->numProfile);
...
void NLDE_DATA_indication(NLDE_DATA_INDICATION_Type *param)
{
  ...
  if (GET_CURRENT_STATE() == HID_STATE_CONFIGURATION)  {
    configurationContextType *context =  (configurationContextType *) hidGlobalContext;
    if ((profileId == ZID_APP_PROFILE) && security && (context->pairRef == param->PairingRef) && 
	(((commandCode == PUSH_ATTRIBUTES_CODE) && isHIDAdapter()) ||
	 ...
	 ((commandCode == SET_REPORT_CODE) && isHIDAdapter()))) {
      ...
      if ((profileId == ZRC_APP_PROFILE) && security && (context->pairRef == param->PairingRef)) {
	context->msgRcv = TRUE;
    ...
    if (GET_CURRENT_STATE() == HID_STATE_IDLE) {
      verifyCmdResponse(param->PairingRef, param->nsdu[0], profileId);
      callbackSchedule(param); 
    }
  }
#ifdef TRACE
...
static void printDebugInfo(NLDE_DATA_INDICATION_Type *param)
{
  uint8_t commandCode, isGDPCmd, profileId;
  ...
  if (profileId == ZRC_APP_PROFILE) {
	...
	DEBUG_MESSAGE(("ZRC_UserControlPressed PairingRef = 0x%02x\r\n", param->PairingRef));
	DEBUG_MESSAGE(("Command Code = 0x%02x\r\n", param->nsdu[1]));
	if (param->nsduLength > 2) {
	  DEBUG_MESSAGE(("Command Payload = "));
	  for (i=0; i<(param->nsduLength-2); i++) {
	    DEBUG_MESSAGE(("0x%02x ", param->nsdu[2+i]));
	  }
	  DEBUG_MESSAGE(("\r\n"));
	}
      }
	...
	DEBUG_MESSAGE(("ZRC_UserControlRepeated PairingRef = 0x%02x\r\n", param->PairingRef));
	DEBUG_MESSAGE(("Command Code = 0x%02x\r\n", param->nsdu[1]));
	if (param->nsduLength > 2) {
	  DEBUG_MESSAGE(("Command Payload = "));
	  for (i=0; i<(param->nsduLength-2); i++) {
    ...
    case USER_CONTROL_RELEASED_CODE:
      {
	DEBUG_MESSAGE(("ZRC_UserControlReleased PairingRef = 0x%02x\r\n", param->PairingRef));
	DEBUG_MESSAGE(("Command Code = 0x%02x\r\n", param->nsdu[1]));
      }
      break;
    case COMMAND_DISCOVERY_REQUEST_CODE:
      {
	DEBUG_MESSAGE(("ZRC_CommandDiscoveryRequest PairingRef = 0x%02x\r\n", param->PairingRef));
      }
      break;
    case COMMAND_DISCOVERY_RESPONSE_CODE:
      {
	...
	DEBUG_MESSAGE(("ZRC_CommandDiscoveryResponse PairingRef = 0x%02x \r\nCommand Supported = ", param->PairingRef));
	for (i=0; i<32; i++) {
	  DEBUG_MESSAGE(("0x%02x ", param->nsdu[2+i]));
	}
	DEBUG_MESSAGE(("\r\n"));
