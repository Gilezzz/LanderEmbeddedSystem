...
/* RF4CE 1.1.0 uses 252 bytes, so if using RF4CE the data storage starts at 0x140
   We allocate 6 HID groups of profile attributes because we have 5 paring table
   entry + the Device profile attributes */
#define NVM_HID_DEV_ATTR_SIZE      126
#define NVM_ADAPTER_ATTR_SIZE      14
#define NVM_ZRC_ATTR_SIZE          40

#define NVM_ZRC_DEV_ATTR_OFFSET    (NVM_MGMT_SIZE_B+0x0140)

#define NVM_HID_DEV_ATTR_OFFSET    (NVM_ZRC_DEV_ATTR_OFFSET+NVM_ZRC_ATTR_SIZE)
#ifdef ZID_FULL
...
#define MAX_ZRC_CMD_SUPPORTED_LENGTH 32
...
typedef struct zrcAttributeS {
  uint32_t aplKeyRepeatInterval;
  uint32_t aplKeyRepeatWaitTime;
  uint8_t  cmdSupported[32];
} zrcAttribute_Type;
#pragma pack()
...
static zrcAttribute_Type zrcAttribute;
...
void ZID_DeviceAttrInit(bool coldStart, uint8_t hidDeviceType)
{
  ...
  if (coldStart) {
    ...
    // ZRC attributes
    zrcAttribute.aplKeyRepeatInterval = aplKeyRepeatInterval_Default;
    zrcAttribute.aplKeyRepeatWaitTime = aplKeyRepeatWaitTime_Default;
    memset(zrcAttribute.cmdSupported, 0, 32);

    ATOMIC(halCommonWriteToNvm((uint8_t*)&zrcAttribute, NVM_ZRC_DEV_ATTR_OFFSET, NVM_ZRC_ATTR_SIZE););
    ...
    halCommonReadFromNvm((uint8_t*)&zrcAttribute, NVM_ZRC_DEV_ATTR_OFFSET, NVM_ZRC_ATTR_SIZE);
    if (HIDDeviceType == HID_CLASS_DEVICE)
...
ATTR_Status ZID_GetDeviceAttr(deviceAttributeInfo_Type *attr)
{
  ...
  switch(attr->id) {
  case aplKeyRepeatInterval_ID:
    {
      uint32_t value;
      value = zrcAttribute.aplKeyRepeatInterval;
      memcpy(attr->value, &value, 4);
      attr->length = 4;      
    }
    break;
  case aplKeyRepeatWaitTime_ID:
    {
      uint32_t value;
      value = zrcAttribute.aplKeyRepeatWaitTime;
      memcpy(attr->value, &value, 4);
      attr->length = 4;  
    }
    break;
...
ATTR_Status ZID_SetDeviceAttr(deviceAttributeInfo_Type *attr)
{
  ...
  case aplKeyRepeatInterval_ID:
    {
      uint32_t value;
      memcpy(&value, attr->value, 4);
      if ((value <= aplcMaxKeyRepeatInterval) && (attr->length == 4)) {
	zrcAttribute.aplKeyRepeatInterval = value;
	flushInfo.zrcFlush = TRUE;
      } else {
  ...
  case aplKeyRepeatWaitTime_ID:
    {
      uint32_t value;
      memcpy(&value, attr->value, 4);
      if ((value >= (2 * aplcMaxKeyRepeatInterval)) && (attr->length == 4)) {
	zrcAttribute.aplKeyRepeatWaitTime = value;
	flushInfo.zrcFlush = TRUE;
      } else {
	status = HID_ATTR_ILLEGAL_REQ;
      } 
...
#ifndef TEST
  if ((status == HID_ATTR_SUCCESS) && (attr->id > aplKeyRepeatWaitTime_ID)) {
      flushInfo.needFlush = TRUE;
  }
#endif
...
/**@brief This function sets th support for the command
  *@param command supported
  *@param command lenght
  *@retval ATTR status
  */
ATTR_Status ZRC_SetCommandSupported(uint8_t *cmdSupported, uint8_t cmdLength)
{
  if (cmdLength > MAX_ZRC_CMD_SUPPORTED_LENGTH) 
    return HID_ATTR_ILLEGAL_REQ;

  memcpy(zrcAttribute.cmdSupported, cmdSupported, cmdLength);
  flushInfo.zrcFlush = TRUE;
  return HID_ATTR_SUCCESS;
}

/**@brief This function gets th support for the command
  *@param command supported
  *@param command lenght
  *@retval ATTR status
  */
ATTR_Status ZRC_GetCommandSupported(uint8_t *cmdSupported, uint8_t *cmdLength)
{
  memcpy(cmdSupported, zrcAttribute.cmdSupported, 32);
  *cmdLength = 32;
  return HID_ATTR_SUCCESS;
}
