...
/**
  * @brief CERC error code: Error code raised if the application attempts to execute a command while 
  * the local device waits an answer from  the remote device (implementation specific)
  */
#define CERC_INVALID_STATE 0xFC01

/**
  * @brief CERC error code: Error code raised on receipt of a successful confirmation 
  * of discovery from the network layer  with more than one node descriptor
  * (implementation specific)
  */
#define CERC_MORE_ONE_NODE 0xFC02

/**
  * @brief CERC return code: Return code raised  on receipt of a queued confirmation 
  * for a data message to sent (implementation specific)
  */
#define CERC_DATA_QUEUED   0xFC03

/**
  * @brief CERC return code: Return code raised on receipt of an unsuccessful 
  * queued for a data message to sent (implementation specific)
  */
#define CERC_DATA_DISCARDED 0xFC04

/**
  * @brief CERC callback define: To signal the user if a discovery confirm is
  * received from the device
  */
#define CERC_DSC_CONFIRM             0x01

/**
  * @brief CERC callback define: To signal the user if a pair confirm is
  * received from the device
  */
#define CERC_PAIR_CONFIRM            0x02

/**
  * @brief CERC callback define: To signal the user if a auto discovery confirm is
  * received from the device
  */
#define CERC_AUTO_DSC_CONFIRM        0x03

/**
  * @brief CERC callback define: To signal the user if a pair indication is
  * received from the device
  */
#define CERC_PAIR_INDICATION         0x04

/**
  * @brief CERC callback define: To signal the user if a comm status indication is
  * received from the device
  */
#define CERC_RF4CE_COMM_STATUS_INDICATION  0x05

/**
  * @brief CERC callback define: To signal the user if a data confirm is received from the device
  */
#define CERC_DATA_CONFIRM         0x06

/**
  * @brief CERC callback define: To signal the user if a unpair indication is
  * received from the device
 */
...
/**
 * @brief (CERC profile attribute) The interval in ms at which user command repeat
 * frames will be transmitted.
 */
#define aplKeyRepeatInterval_ID 0x80

/**
  * @brief (Default value)  The interval in ms at which user command repeat frames will be
  * transmitted. Valid range 0 - aplcMaxKeyRepeatInterval
  */
#define aplKeyRepeatInterval_Default 0xC35 /* 50ms */

/**
  * @brief (CERC profile attribute) The duration that a recipient of a user
  * control repeated command frame waits before terminating a repeated operation.
  */
#define aplKeyRepeatWaitTime_ID 0x81

/**
 * @brief (Default value) The duration that a recipient of a user control repeated
 * command frame waits beforeterminating a repeated operation. 
 * Valid range >= aplcMaxKeyRepeatInterval
 */
#define aplKeyRepeatWaitTime_Default aplcMaxKeyRepeatInterval /* 100ms */

/**
  * @brief (CERC profile attribute) The number of key exchange transmissions used to generate 
  * a security key. Valid range aplcMinKeyExchangeTransferCount – 0xff
  */
#define aplKeyExchangeTransferCount_ID 0x82

/**
  * @brief (Default value) The number of key exchange transmissions 
...
typedef struct CERC_Init_S {
  ...
  /** The duty cycle of a device in MAC symbols. A value
...
typedef struct CERC_DscPair_S {  
  /** Flag to signal if the discovery and Pair procedure is executed between two TARGETs.
    * If the flag is set the Target send a discovery and pair (like a Controller) 
    */
  bool pairTT;
  ...
  /** The device type to discover. This value can be set to 0xff 
    * to indicate a wildcard.  (Only for Discovery request)
    */
  uint8_t searchDevType;

  /** Number of Key exchange (target to controller) secured pairing
    */
  uint8_t numberOfKey;
} CERC_DscPair_Type;
