...
/**@brief  Read a character
  *@param  character 
  *@retval TRUE if there was a char, FALSE otherwise
  */
boolean serialReadByte(uint8_t *ch)
{
  if (rxBottom != rxTop) {
    *ch = rxQ[rxBottom];  
    rxBottom = (rxBottom+1) % sizeof(rxQ);
    return TRUE;
  } else
    return FALSE;
}

/**@brief  Init the queue used ti store the char pressed by the user
  *@param  None
  *@retval None
  */
void printQueueInit(void)
{
  rxBottom = 0;
  rxTop = 0;
}
